...
fact { all user1, user2: User | (user1 != user2) => (user1.ssn != user2.ssn)}
...
fact { all company1, company2: Company |(company1 != company2) => company1.api != company2.api}
...
fact { all company: Company | all r1, r2 : DataAccessRequest |
 	  (r1 in company.requests && r2 in company.requests) => (r1 != r2)}

//all subscription in Company.subscriptions are different
fact { all company: Company | all s1, s2 : Subscription |
 	  (s1 in company.subscriptions && s2 in company.subscriptions) => (s1 != s2)}
